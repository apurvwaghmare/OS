...
// Function to find the index of the MRU page
int findMRU(int time[], int n) {
    int maximum = time[0], pos = 0;
    for (int i = 1; i < n; ++i) {
        if (time[i] > maximum) {
            maximum = time[i];
            pos = i;
...
int mruPageFaults(int pages[], int n) {
    int frames[PAGE_FRAMES];
    int time[PAGE_FRAMES];
    int pageFaults = 0, counter = 0;
    ...
    for (int i = 0; i < n; i++) {
        if (!isPageInFrames(frames, pages[i])) {
            int mruIndex = findMRU(time, PAGE_FRAMES);
            frames[mruIndex] = pages[i];
            time[mruIndex] = counter++;
            pageFaults++;
        } else {
            for (int j = 0; j < PAGE_FRAMES; j++) {
                if (frames[j] == pages[i]) {
...
int main() {
    ...
    int mruFaults = mruPageFaults(pages, n);
    ...
    printf("Total page faults using MRU: %d\n", mruFaults);
